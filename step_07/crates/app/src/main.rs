...
use cadapter_file::{FileInput, FileOutput};
...
fn main() -> Result<()> {
    println!("=== Greeting Service (Step 07 - Modular Monolith & Hexagonal Architecture) ===");
    ...
    if args.once {
        ...
        service
            .run_greeting_loop(input.as_ref(), output.as_ref())
